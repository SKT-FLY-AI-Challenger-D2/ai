...
* `legal_issue_evidence`는 핵심 근거 2개만 주세요. 이때 법률명을 표기할때는 반드시 제공된 법률/규정 근거 내 [법률명] 처럼 표기하고, ( 호 / 항 / 조 / 목 ) 은 제외하세요.
* 출력 형식은 아래의 Json 형식 예시를 참고하세요.
```json
{{
  "legal_issue_score": 0.8,
  "legal_issue_evidence": ["[식품표시광고법] 일반 식품을 혈당 조절 효능이 있다고 표방, 위반 가능성 있음", "[표시광고법] 소비자 오인 유발 과장 광고, 위반 가능성 있음"]
}}
```
"""
