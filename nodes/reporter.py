...
def reporter_node(state: ModerationState) -> dict:
    ...
    try:
        ...
        prompt = f"""
        당신은 전문 콘텐츠 모니터링 분석가입니다. 아래의 분석 데이터를 바탕으로 전체적인 moderation 상태를 보고하는 종합 보고서를 작성해주세요.
        
        보고서는 다음 지침을 따라야 합니다:
        1. **줄글 형태**로 읽기 좋고 전문적으로 작성할 것.
        2. 법적 문제, 딥페이크/AI 생성 여부, 팩트 체크 결과를 종합적으로 분석할 것.
        3. 단순 수치 나열보다는 각 분석 결과가 의미하는 바를 설명할 것.
        4. 최종 위험도({final_score:.2f}/1.0)에 따른 결론을 명확히 제시할 것.
        5. 한국어로 작성할 것.
        6. 12줄 정도로 작성할 것.
